...
export default function ListPage() {
  ...
  const data = useAppSelector(selectARTList);
  const totalRow = useAppSelector(selectARTTotalRow);
  const pageCount = useAppSelector(selectARTPageCount);
  const filter = useAppSelector(selectARTFilter);
  const loading = useAppSelector(selectARTLoading);
  ...
  useEffect(() => {
    dispatch(artActions.fetchARTList(filter));
  }, [dispatch, filter]);
  ...
  const handleFilterChange = (newFilter: ListParams) => {
    dispatch(artActions.setFilter(newFilter));
  };
      ...
      <div style={{ padding: 20 }}>
        <Box className={classes.titleContainer}>
          <Typography component="h1" variant="h5" fontWeight="bold">
            {t('ART Histories management')}
          </Typography>
        </Box>
        {/*
          Filter
        */}
        <ARTFilter filter={filter} onChange={handleFilterChange} siteList={siteList} />
        {/*
          Loading 
        */}
        <br />
        ...
        */}
        ...
        {/*
          Table
        */}
        <ARTTable data={data} startIndex={filter.pageSize * filter.pageIndex} />
        {/*
          Pagination
        */}
        <Toolbar style={{ backgroundColor: 'white' }}>
                      ...
                      if (uploadFiles) {
                        for (let index = 0; index < uploadFiles.length; index++) {
                          ...
                          try {
                            ...
                            const response = await artApi.importFile(item.file);
                            if (response.succeed) {
